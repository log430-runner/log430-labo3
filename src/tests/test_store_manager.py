...
def test_stock_flow(client):
    ...
    check_stock(client, product_id, 3)
    ...
    # check if stock is back to 5:
    check_stock(client, product_id, 5)

def check_stock(client, prod_id, quantity):

    response = client.get(f'/stocks/{prod_id}')
